...
export default function CatchWritePage() {
  ...
  async function handleSubmit() {
    ...
    const { error: insertError } = await supabase.from('catch_reports').insert({
  ...
  return (
    <div className="max-w-2xl mx-auto min-h-screen pb-24 px-4" style={{ background: '#0D1F35', paddingTop: '88px' }}>
      ...
      <div className="rounded-2xl p-5 space-y-4" style={{ background: '#1A3355', border: '1px solid rgba(45,95,153,0.4)' }}>
        ...
        {/* 내용 */}
        <div>
          <label className="text-xs font-medium block mb-1.5" style={{ color: '#93B5D4' }}>내용 (선택)</label>
          <textarea
            rows={5}
