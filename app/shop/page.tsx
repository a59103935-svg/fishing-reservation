    ...
    <div className="max-w-sm mx-auto min-h-screen pb-36" style={{ background: '#0D1F35' }}>
            ...
            {filtered.map((product) => {
              ...
              return (
                <button
                  ...
                  className="rounded-2xl overflow-hidden text-left flex flex-col transition-all active:scale-95 hover:border-gold"
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40">
        ...
        >
          ...
          <button
            onClick={() => items.length > 0 && setShowForm(true)}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-end">
          ...
          <div
            className="relative w-full max-w-sm mx-auto rounded-t-3xl px-5 pt-5 pb-8"
            style={{ background: '#0F1E30', border: '1px solid rgba(45,95,153,0.5)', borderBottom: 'none' }}
          >
            <div className="w-10 h-1 rounded-full mx-auto mb-5" style={{ background: 'rgba(45,95,153,0.5)' }} />
            <h2 className="text-base font-bold mb-1" style={{ color: '#F8F9FA' }}>주문자 정보</h2>
            <p className="text-xs mb-4" style={{ color: '#4A6888' }}>출조 시 이름/연락처로 수령 확인합니다</p>

            <div className="space-y-3 mb-4">
              <div>
                <label className="text-xs font-medium block mb-1" style={{ color: '#4A6888' }}>이름</label>
                <input
                  type="text"
                  placeholder="홍길동"
                  value={guestName}
              ...
              <div>
                <label className="text-xs font-medium block mb-1" style={{ color: '#4A6888' }}>연락처</label>
                <input
                  type="tel"
            ...
            <div className="rounded-xl px-4 py-3 mb-4 space-y-1.5" style={{ background: 'rgba(30,63,102,0.5)' }}>
              {items.map((item) => (
                <div key={item.product.id} className="flex justify-between text-xs">
                  <span style={{ color: '#93B5D4' }}>{item.product.name} × {item.quantity}</span>
                  <span style={{ color: '#C9A84C' }}>{formatPrice(item.product.price * item.quantity)}</span>
                </div>
              ))}
              <div className="flex justify-between font-bold pt-1.5" style={{ borderTop: '1px solid rgba(45,95,153,0.4)' }}>
                <span className="text-sm" style={{ color: '#F8F9FA' }}>합계</span>
                <span className="text-sm" style={{ color: '#C9A84C' }}>{formatPrice(cartTotal)}</span>
              </div>
            </div>
